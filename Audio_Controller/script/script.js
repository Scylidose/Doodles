...
var playlist = ["musique/65daysofstatic - Debutante (No Mans Sky).mp3", "musique/No Mans Sky OST - Supermoon.mp3"];
 ...
 */
function jouer() {
    $("#changement").replaceWith("<svg  xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='9px' height='10px' class=«pause» id='changement'><path fill-rule='evenodd'  fill='rgb(3, 3, 4)' d='M4.000,10.000 L4.000,-0.000 L6.000,-0.000 L6.000,10.000 L4.000,10.000 ZM-0.000,-0.000 L2.000,-0.000 L2.000,10.000 L-0.000,10.000 L-0.000,-0.000 Z'/></svg>");
 ...
 */
function stop() {
    $("#changement").replaceWith("<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='9px' height='10px' class=«play» id='changement' onclick='document.getElementById('player').play(); jouer();'> <path fill-rule='evenodd' fill='rgb(3, 3, 4)' d='M-0.000,10.000 L9.004,4.998 L-0.000,-0.004 L-0.000,10.000 Z' /> </svg>");
...
/**
 * Fonction permettant de récupérer le titre à partir
 * du tableau contenant les chemins des pistes audio 
 */
function trier_titre() {
    for (j = 0; j < playlist.length; j++) {
            titre[j] = playlist[j].replace("musique/", "").replace(".mp3", "");
    }
}

/**
 * Fonction permettant d'afficher le titre
 * dans la div avec l'id 'title'
 */
